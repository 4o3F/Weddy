package moe.exusiai.handler.messagefilter.algorithm;

import moe.exusiai.handler.messagefilter.MessageFilter;
import moe.exusiai.handler.messagefilter.algorithm.utils.StringLengthComparator;
...
public class SensitiveWordInit {
    ...
    private Set<String> readSensitiveWordFile() throws Exception{
        ...
        InputStreamReader read = new InputStreamReader(this.getClass().getClassLoader().getResourceAsStream("SensitiveWord.txt"),ENCODING);
        try {
            ...
            while((txt = bufferedReader.readLine()) != null){    //读取文件，将文件内容放入到set中
                ...
                if (decodeTxt.length()==1) { // 先判断是否为单字（DFA无法处理单字）
                    MessageFilter.banCharList.add(decodeTxt.toLowerCase());
                } else{ // 则若解码后的非单关键字不在忽略列表中，则添加至hashMap
                    set.add(decodeTxt.toLowerCase());
                }
                // 从config读取的词汇录入
//                List<String> configAddList = MessageFilter.snowkPlugin.getConfig().getStringList("add");
